...
struct Manifest {
  ...
  static Manifest load(const fs::path& model_directory) {
    ...
    if (result.schema_version >= 3) {
      ...
      const std::map<std::string, std::string> expected_preprocessing = {
          {"version", "opencv_imread_bgr2rgb_inter_area_stretch_imagenet_nchw_float32_v1"},
      ...
      if (!preprocessing.is_object() ||
          preprocessing.size() != expected_preprocessing.size()) {
        throw std::runtime_error("Unsupported preprocessing contract");
      }
      for (const auto& [field, expected] : expected_preprocessing) {
 ...
 private:
  ...
  std::vector<float> preprocess(const fs::path& image_path) const {
    ...
    cv::resize(rgb, resized,
               cv::Size(manifest_.image_size, manifest_.image_size), 0.0, 0.0,
               cv::INTER_AREA);
